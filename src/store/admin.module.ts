...
export const adminModule: Module<any, any> = {
  namespaced: true,
  state: {
    user: null,
    token: null,
  ...
  mutations: {
    ...
    REMOVE_TAB(state, tabName) {
      let idx = state.tabs.findIndex((tab: any) => tab.name === tabName)
    ...
    UPDATE_TAB(state, obj) {
      let idx = state.tabs.findIndex((tab: any) => tab.name === obj.name)
  ...
  actions: {
    login({dispatch, commit}, usermsg) {
      return api.public.login(usermsg).then((data: any) => {

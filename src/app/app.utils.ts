...
export default {
  isChanged (obj: any, copyobj: any): boolean {
  ...
  formatComboboxData (data: Array<any>): any[] {
    return data.map(d => ({
      value: d.code,
      text: d.name,
    }))
  },
  confirm (msg: string, method: string): Promise<any> {
  ...
  getTreePathByKey (value: string, key: string, arr: string[]): any[] | undefined {
    let temppath: string[] = []
    try {
      let getNodePath = (node: any) => {
